...
## 참고 실험 — 파인튜닝 없이 프롬프트 1줄 추가 (2026-07-22)

**배경**: v3 결론에서 미적용으로 남겨둔 후보 (a) "원문 직접인용 유도"를, 파인튜닝 없이 순수
프롬프트 실험으로 먼저 저비용 검증. `scripts/raft_data_gen_v3_fullregen_unused.py`의 `good_sys`
(학습데이터 생성용 GPT 프롬프트)에 이미 구현은 돼 있었으나 실제 학습 데이터에는 한 번도 반영된
적 없던 지시문을, 그대로 **실전 답변 시스템 프롬프트**로 옮겨 베이스 모델(파인튜닝 안 함)에 적용.

**방법**: `rag/graph.py`는 전혀 수정하지 않고, 평가 스크립트 안에서 `Pipeline._answer_system_prompt`를
프로세스 내부에서만 monkey-patch(읽기 전용 계측과 동일 패턴). 기존 `_answer_system_prompt()`가
반환하는 문자열 끝에 아래 긍정 지시 1개만 추가(부정 지시가 refusal을 급증시킨다는 v1/v2 교훈을
반영해 긍정 지시만 사용):

> "핵심 결론을 말하기 전에, 그 결론의 근거가 되는 근거 원문 문장을 [식별자] 인용과 함께 그대로
> 한 번 옮겨 적은 뒤 결론을 서술하라(원문을 바꿔 쓰지 말 것)."

나머지 조건(held-out 30문항, seed=42, 실전 검색 파이프라인 route→retrieve→[rewrite 재시도]→
answer→verify, 판사 GPT-4o-mini)은 베이스라인/v1/v2/v3 평가와 완전히 동일 — 직접 비교 가능.

**결과**:

| | 거부율 | Faithfulness | Answer Relevancy |
|---|---|---|---|
| 베이스라인(원본 프롬프트) | 33.0% (10/30) | 0.900 | 0.975 |
| 베이스 + 프롬프트 1줄 | **13.3%** (4/30) | 0.858 | 0.931 |

거부율은 베이스라인보다도 19.7%p 낮아졌다(모델에게 "확신 판단"이라는 애매한 과제 대신 "관련
문장을 찾아 옮겨 적기"라는 구체적 과제를 줬기 때문으로 해석). 다만 답변한 26건 중 8건(30.8%)에
unsupported claim이 남아, Faithfulness·Answer Relevancy는 둘 다 베이스라인보다 낮아졌다 —
이전엔 거부했을 애매한 문항까지 답을 시도하면서 평균이 끌어내려진 것으로 해석된다.

**베이스라인 대비 결론**: "답변한 문항만"의 조건부 평균으로는 Faithfulness·Answer Relevancy가
하락해 베이스라인을 못 넘는 것처럼 보인다. 그러나 이 조건부 평균은 거부(=사용자 입장에서 가치 0)를
분모에서 빼버려 그 손실을 반영하지 못한다. 거부율까지 곱해 **30문항 전체 기준**으로 다시 계산하면:
...
**이 기준으로는 프롬프트 추가판이 베이스라인까지 포함해 전부를 이긴다.** 답변 품질은 소폭
하락해도 거부를 훨씬 덜 하는 쪽이, 사용자에게 전달되는 전체 가치로는 더 크다는 뜻.

**파인튜닝 3회(v1/v2/v3) 대비 결론**: 거부율·Faithfulness·Answer Relevancy **세 지표 모두**에서
v1/v2/v3 전부를 앞섰다(v3 대비 거부율 43.3%→13.3%, Faithfulness 0.794→0.858, Answer Relevancy
0.724→0.931). 즉 GPU 파인튜닝 3회로도 못 넘은 벽을, 학습 없이 프롬프트 한 줄로 더 가깝게
다가갔다 — QLoRA 자체가 이 문제(근거 이탈)에 잘 안 맞는 도구였을 가능성을 시사한다.

**부수 확인**: golden 근거가 실제 검색(top-8)에 잡힌 비율이 8/30(26.7%)로 v2·v3와 정확히 일치 —
검색 파이프라인은 로컬모델/프롬프트 종류와 무관하게 항상 동일하게 동작한다는 걸 실측으로 확인
(이전까지는 구조적 추론이었으나 이번에 처음 직접 측정으로 확정).

**재현**: `eval/exaone_baseline_quotefirst_eval.py`, 원시 결과
`eval/results/exaone_baseline_quotefirst_results.jsonl`.

**후속 — v3(파인튜닝판)에 같은 프롬프트를 추가로 얹으면?** (2026-07-22, `eval/exaone_v3_quotefirst_eval.py`)

파인튜닝판 v3 위에 동일한 프롬프트 1줄을 똑같이 monkey-patch로 얹어 같은 30문항으로 재평가했다
(나머지 조건 전부 동일, `local_model="exaone3.5-lora:v3"`만 다름).
...
v3 단독 대비로는 거부율(43.3%→10.0%)과 Faithfulness(0.794→0.833) 둘 다 개선됐고, Answer
Relevancy는 거의 그대로(0.724→0.719)다. 프롬프트 트릭은 파인튜닝 모델 위에서도 그대로 작동한다.

같은 방식(거부율까지 곱한 30문항 전체 기준)으로 베이스+프롬프트와 직접 비교하면:

| | Faithfulness(전체 30문항, 거부=0점) | Answer Relevancy(전체 30문항, 거부=0점) |
|---|---|---|
| v3(원본 프롬프트) | 0.794×56.7% = 0.450 | 0.724×56.7% = 0.410 |
| v3 + 프롬프트 1줄 | 0.833×90.0% = **0.750** | 0.719×90.0% = 0.647 |
| 베이스 + 프롬프트 1줄 | 0.858×86.7% = 0.744 | 0.931×86.7% = **0.807** |

**결론 — "파인튜닝을 더 얹으면 개선될 여지가 있는가"**: 있기는 하지만 엇갈린다. v3+프롬프트는
Faithfulness에서 베이스+프롬프트를 근소하게 앞서고(0.750 vs 0.744) 거부율은 전 실험 통틀어
가장 낮지만(10.0%), Answer Relevancy에서는 크게 뒤진다(0.647 vs 0.807). 즉 파인튜닝을 추가로
얹는 게 손해는 아니지만(단독 v3보다는 전부 개선), 프롬프트 단독(베이스+프롬프트)을 이기지는
못한다 — 특히 "질문에 실제로 답했는가"를 재는 Answer Relevancy에서 파인튜닝이 여전히 발목을
잡는다는 뜻으로, v2·v3의 근본 원인 분석(품질보다 형식에 치우친 학습 신호)과 일관된다.

**부수 확인**: golden 근거 hit-rate는 이번에도 8/30(26.7%)로 동일 — 검색 파이프라인은 어떤
로컬모델/프롬프트를 쓰든 완전히 무관하게 동작한다는 걸 5번째 실험에서도 재확인.
